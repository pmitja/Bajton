      ...
      <SheetContent className="w-full overflow-y-auto p-0 sm:max-w-xl">
        ...
        <form ref={formRef} action={formAction} className="space-y-6 px-6 py-6">
          ...
          <div className="rounded-xl border border-dashed bg-muted/35 p-3">
            {process.env.NEXT_PUBLIC_UPLOADTHING_ENABLED === "true" ? (
              ...
              <div className="flex min-h-40 flex-col items-center justify-center text-center">
                ...
                <p className="mt-3 text-xs text-amber-700">Upload se aktivira po nastavitvi svežega UploadThing tokena.</p>
              </div>
            )}
          </div>
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div className="space-y-2"><Label htmlFor="invoiceDate">Datum računa</Label><Input id="invoiceDate" name="invoiceDate" type="date" required aria-invalid={Boolean(state.errors?.invoiceDate)} />{state.errors?.invoiceDate ? <p className="text-xs text-destructive">{state.errors.invoiceDate[0]}</p> : null}</div>
            ...
            <div className="space-y-2 sm:col-span-2"><Label htmlFor="note">Opomba</Label><Input id="note" name="note" placeholder="Kratek kontekst za ostale investitorje" /></div>
          </div>
